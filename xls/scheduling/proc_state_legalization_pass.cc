...
absl::StatusOr<bool> AddDefaultNextValue(Proc* proc,
  ...
  for (Next* next : proc->next_values(state_element)) {
    ...
    if (predicate->OpIn({Op::kAnd}) && predicate->operands().size() == 2) {
      for (StateRead* state_read : state_reads) {
        if (!state_read->predicate().has_value()) {
          continue;
        }
        // Check to see if this is just an `and` with the state read predicate.
        // If so, take the other operand & see if it's a not/nor of the other
        // conditions.
        if (predicate->operand(0) == *state_read->predicate()) {
          predicate = predicate->operand(1);
          break;
        }
        if (predicate->operand(1) == *state_read->predicate()) {
