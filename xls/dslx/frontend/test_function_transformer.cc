...
absl::StatusOr<Function*> CreateNewFunction(
    ...
    const std::vector<const NameDef*>& sorted_defs_to_promote,
    const absl::flat_hash_map<const NameDef*, std::string>&
        original_to_unique_name,
    const absl::flat_hash_map<const NameDef*, NameDef*>&
        original_to_cloned_local_def) {
  ...
  for (const NameDef* def : sorted_defs_to_promote) {
    std::string unique_name = original_to_unique_name.at(def);
    NameDef* cloned_local_def = original_to_cloned_local_def.at(def);
    NameRef* local_ref =
...
absl::flat_hash_set<const NameDef*> find_promoted(
    const absl::flat_hash_set<const NameDef*>& name_defs,
    const absl::flat_hash_set<const NameRef*>& name_refs) {
...
absl::StatusOr<Impl*> TestFunctionTransformer::TransformToTestProc(
  ...
  // Partition the test function statements into constexpr (setup) and
  // non-constexpr (runtime) statements.
  //
  // Safety of reordering:
  // We move all constexpr statements to the proc `new` function (init) and
  // non-constexpr statements to the proc `next` function. This effectively
  // reorders them, executing all constexpr statements first.
  // This is safe because:
  // 1. Constexpr statements cannot depend on non-constexpr statements (the
  //    type checker enforces this).
  // 2. DSLX constexpr statements are side-effect free (except for defining
  //    names), so executing them early does not change behavior.
  // 3. Any variables defined in constexpr statements and used in non-constexpr
  //    statements will be promoted to proc fields, preserving their values
  //    across the transition from `new` to `next`.
  ...
  // But their names need to be uniquified.
  absl::flat_hash_set<const NameDef*> defs_to_promote =
      find_promoted(def_finder.name_defs(), ref_finder.name_refs());
  ...
  absl::flat_hash_map<const NameDef*, NameDef*> original_to_member_name_def;
  ...
  for (const NameDef* def : sorted_defs_to_promote) {
    ...
    original_to_member_name_def[def] = member_name_def;
  ...
  // Clone setup statements with renaming replacer
  ...
  auto new_replacer =
    ...
    if (const auto* name_def = dynamic_cast<const NameDef*>(node)) {
      if (defs_to_promote.contains(name_def)) {
        std::string unique_name = original_to_unique_name.at(name_def);
    ...
    if (const auto* name_ref = dynamic_cast<const NameRef*>(node)) {
      if (std::holds_alternative<const NameDef*>(name_ref->name_def())) {
        const NameDef* original_def =
            std::get<const NameDef*>(name_ref->name_def());
        if (defs_to_promote.contains(original_def)) {
          std::string unique_name = original_to_unique_name.at(original_def);
  ...
  XLS_ASSIGN_OR_RETURN(
      ...
      CreateNewFunction(fn, new_module, proc_def_type,
                        ...
                        cloned_const_statements, sorted_defs_to_promote,
                        original_to_unique_name, original_to_cloned_local_def));

  XLS_ASSIGN_OR_RETURN(
      Function * next_fn,
      CreateNextFunction(fn, new_module, proc_def_type, non_const_statements,
                         defs_to_promote, original_to_unique_name, type_info_));
